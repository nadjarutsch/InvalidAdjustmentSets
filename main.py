...
def extract_graph(graph_config: DictConfig) -> nx.DiGraph:
    ...
    Returns:
    - nx.DiGraph: A directed graph representing the causal relationships between variables.
    """
    graph = nx.DiGraph()
...
def generate_data(variables: list, cfg: DictConfig, seed: int) -> np.ndarray:
    """
    Generates synthetic data from a structural causal model given in the Hydra configuration.

    Parameters:
    - variables (list): List of variable names in causal order.
    - cfg (DictConfig): Hydra configuration object containing structural equations.
    - seed (int): Random seed for reproducibility of additive Gaussian noise.

    Returns:
    - np.ndarray: An array of shape (len(variables), sample_size). Data is generated by adding Gaussian
                noise to values computed from the structural equations.
    """
...
def main(cfg: DictConfig) -> None:
    """
    Runs experiments based on a Hydra configuration.
